  ...
  async play(song, message, client, filters) {
    ...
    if (!song) {
      ...
      const endembed = new MessageEmbed().setColor("#c219d8")
        .setAuthor(`Antrian Musik berakhir.`)
  ...
  let thumb;
    ...
    try {
      const newsong = new MessageEmbed()
        ...
        .addField("Durasi:", `\`${song.duration} Menit\``, true)
    ...
    collector.on("collect", async (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        ...
        let nowPlaying = new MessageEmbed()
          .setTitle("Sedang Memainkan!")
          .setDescription(`[**${song.title}**](${song.url})`)
          .setThumbnail(song.thumbnail.url)
          .setColor("#1c00c2")
          .setFooter("Waktu yang tersisa: " + new Date(left * 1000).toISOString().substr(11, 8));
          //if its a stream
          if(ms >= 10000) {
        ...
        case "⏭":
          ...
          const skipembed = new MessageEmbed().setColor("#1ace00").setAuthor(`${user.username} Melompati Lagu.`)
        ...
        case "📑":
          ...
          try {
            ...
            if (!lyrics) lyrics = `Tidak menemukan lirik untuk ${queue.songs[0].title}.`;
          } catch (error) {
            lyrics = `Tidak menemukan lirik untuk ${queue.songs[0].title}.`;
          }
        ...
        case "⏯":
          ...
          if (queue.playing) {
            ...
            const pausemebed = new MessageEmbed().setColor("#1c00c2")
              .setAuthor(`${user.username} Menjeda Musik!.`)
            queue.textChannel.send(pausemebed).catch(console.error);
          } else {
            queue.playing = !queue.playing;
            queue.connection.dispatcher.resume();
            const playembed = new MessageEmbed().setColor("#1ace00")
              .setAuthor(`${user.username} Memutar kembali Muisik!`)
        ...
        case "🔄":
          ...
          const loopembed = new MessageEmbed().setColor("#1ace00")
        ...
        case "⏹":
          ...
          if (!canModifyQueue(member)) return;
          queue.songs = [];
          const stopembed = new MessageEmbed().setColor("#1c00c2").setAuthor(`${user.username} Musik Berhenti!`)
