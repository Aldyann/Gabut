...
module.exports = {
  name: "radio",
  description: "Mainkan Stasiun Radio",
  cooldown: 3,
  edesc: `Ketik perintah ini untuk memutar siaran langsung radio!\nPenggunaan: ${PREFIX}radio <1-34>`,
 async execute(message, args, client) {
  //define the No args Embed, lmao
  let resultsEmbed = new Discord.MessageEmbed()
      .setTitle(`**✅ Stasiun Radio yang Tersedia**`)//
      .addFields(
        { name: `*** 📻  Radio Standar***`, value: `**1:  ** [\`${Radiostations[1-1].split(" ")[0]}\`](${Radiostations[1-1].split(" ")[1]})
        ...
        **5:  ** [\`${Radiostations[5-1].split(" ")[0]}\`](${Radiostations[5-1].split(" ")[1]})
        ` , inline: true}, { name: `*** 📻 Radio Standar***`, value: `**6:  ** [\`${Radiostations[6-1].split(" ")[0]}\`](${Radiostations[6-1].split(" ")[1]})
...
**34: ** [\`${Radiostations[34-1].split(" ")[0]}\`](${Radiostations[34-1].split(" ")[1]})`, inline: true },
  ...
  if(!message.guild)      
    ...
    //if no args
    if (args[0] == null) {
      message.channel.send(    new MessageEmbed().setColor("#1c00c2")
      .setDescription(`**👍 ${message.author} Cek \`DM\` kamu untuk melihat daftar stasiun radio!**`)
       );
       message.author.send(new MessageEmbed().setColor("#1c00c2")
  ...
  //react with emoji
    ...
    if (!permissions.has("CONNECT"))
      return attentionembed(message,"Saya perlu izin untuk bergabung dengan saluran Anda!");
    ...
    if(isNaN(args[0])) {
      ...
      new MessageEmbed()
      .setColor("#RED")
      .setTitle( `Bukan stasiun radio yang valid, harap gunakan Nomor antara \`1\` dan \`${Radiostations.length}\``)
     );}
...
.setDescription(`**👍 Bergabung \`${channel.name}\` 📄 Melompati \`#${message.channel.name}\`**`)
.setFooter(`${message.author.username}#${message.author.discriminator}`));
//send Search something embed
message.channel.send(new MessageEmbed().setColor("#1c00c2")
.setDescription(`**💢 Mencari 🔍 \`${Radiostations[i-1].split(" ")[0]}\`**`));
//mute yourself
...
if (serverQueue) {
  ...
  if (estimatedtime > 60) {
    ...
    estimatedtime = estimatedtime + " Menit"
  }
  else if (estimatedtime > 60) {
    estimatedtime = Math.round(estimatedtime / 60 * 100) / 100;
    estimatedtime = estimatedtime + " Jam"
  }
  else {
    estimatedtime = estimatedtime + " Detik"
  }
  ...
  const newsong = new MessageEmbed()
    ...
    .setURL(song.url)
    .setDescription(`\`\`\`Telah ditambahkan ke Antrian.\`\`\``)
    .addField("Perkiraan waktu sampai bermain:", `\`${estimatedtime}\``, true)
    .addField("Posisi dalam antrian", `**\`${serverQueue.songs.length - 1}\`**`, true)
    .setFooter(`Request dari: ${message.author.username}#${message.author.discriminator}`, message.member.user.displayAvatarURL({ dynamic: true }))
...
try {
  ...
  await channel.leave();
  return message.channel.send(`Tidak dapat bergabung dengan saluran: ${error}`).catch(console.error);
}
  //sende bestätigung
 }
};
